...
app.get('/getVenues', function (req, res) {
    ...
    request('https://api.foursquare.com/v2/venues/search?client_id=BHY5FVRSQXV3ERYBXPEKKOYCRXLXYKPE1QA11ET1C5CT2JS1&client_secret=DW20QQLZTYX4CQYHUKIELNEDOZZX0XEN2X4LGYECDEPW035I&v=20160215&limit=5&near=Kansas&query=pizza', function (error, response, body) {
        //Check for error
        if(error){
...
app.post('/register', function (req, res) {
    ...
    var insertDocument = function (db, callback) {
        db.collection('aselab9').insertOne(response, function (err, result) {
    ...
    app.post('/get-data', function (req, res) {
        ...
        var findrestarunt = function (db, callback) {
            var cursor = db.collection('aselab9').find();
    ...
    app.post('/update', function (req, res) {
        ...
        var updateDocument = function (db, data, callback) {
            db.collection('aselab9').updateOne({"data.response.venues[i].name": ven[i].name}, {$set: item}, function (err, result) {
                if (err) {
                    res.write(" Failed");
                    res.end();
                }
    ...
    app.post('/delete', function (req, res) {
        ...
        MongoClient.connect(url, function (err, db) {
            ...
            db.collection('aselab9').deleteOne({"data.response.venues[i].name": ven[i].name}, function (err, result) {
